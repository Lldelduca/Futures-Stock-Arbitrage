...
def trade_pair(stock_id, stock_id_dual, position_dual,counter_greedy, counter_guaranteed):
    ...
    if not (stock_order_book and stock_order_book.bids and stock_order_book.asks):
        ...
        time.sleep(0.2)
        return counter_greedy, counter_guaranteed
    else:
    ...
    if stock_order_book_dual.asks:
        ...
        if best_bid_price > best_ask_price_dual:  # dual undervalued → buy dual
            side = "bid"
            price = max(best_ask_price_dual,best_bid_price-increment)
            if is_close(price, best_bid_price - increment):
                counter_guaranteed=counter_guaranteed+1
            else:
                counter_greedy= counter_greedy+1
    ...
    if stock_order_book_dual.bids:
        ...
        if best_ask_price < best_bid_price_dual:  # dual overvalued → sell dual
            side = "ask"
            price = min(best_bid_price_dual,best_ask_price+increment)
            if is_close(price, best_ask_price + increment):
                counter_guaranteed=counter_guaranteed+1
            else:
                counter_greedy= counter_greedy+1
...
def hedge_pair(stock_id, stock_id_dual, outstanding_position):
    if outstanding_position > 0:
        stock_order_book = exchange.get_last_price_book(stock_id)
        if not (stock_order_book and stock_order_book.bids):
            print(f"Order book for {stock_id} has no bids. Skipping.")
            time.sleep(0.2)
            return
        ...
        if not (stock_order_book and stock_order_book.asks):
            print(f"Order book for {stock_id} has no asks. Skipping.")
            time.sleep(0.2)
            return
    ...
    if not trade_would_breach_position_limit(stock_id, volume_hedge, side_hedge):
        print(
            f"Inserting {side_hedge} for {stock_id}: {volume_hedge:.0f} lot(s) at price {price_hedge:.2f}."
        )
        exchange.insert_order(
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Inner loop: trade + hedge repeatedly for 60 seconds
        ...
        while True:
            ...
            # Run strategy for all pairs
            positions = exchange.get_positions()
            for stock_id, stock_id_dual in PAIRS:
                position_dual = positions[stock_id_dual]
                counter_greedy, counter_guaranteed = trade_pair(
                    stock_id, stock_id_dual, position_dual, counter_greedy, counter_guaranteed
                )
                trade_count += 1
                traded_volume += 1
            ...
            positions = exchange.get_positions()
            for stock_id, stock_id_dual in PAIRS:
                outstanding_position = positions[stock_id] + positions[stock_id_dual]
                hedge_pair(stock_id, stock_id_dual, outstanding_position)
                trade_count += 1
                traded_volume += 1

            time.sleep(0.2)

        # PnL update once per minute
    ...
    print("=" * 60)
    print("counter_greedy :", counter_greedy)
    print("counter_guaranteed :", counter_guaranteed)
