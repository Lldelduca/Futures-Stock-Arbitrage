...
def trade_pair_future(stock_id_future, stock_id_future_2, discount_factor, discount_factor_2):
    ...
    if side != "none":
        ...
        if future_vol_2 > 0:
            ...
            return side, future_vol_2+future_vol
...
def hedge(stock_id, stock_id_future, stock_id_future_2, d_f, d_f_2, trade_side):
    ...
    outstanding_position = round(positions[stock_id_future]*d_f + positions[stock_id_future_2] * d_f_2  + trade_adjustment + positions[stock_id])

    if outstanding_position > 0:
        book = exchange.get_last_price_book(stock_id)
        if not (book and book.bids):
            print(f"{stock_id} has no bids. Skipping cover.")
            return 0
        price = book.bids[0].price
        side = "ask"
        volume = outstanding_position
    elif outstanding_position < 0:
        book = exchange.get_last_price_book(stock_id_future)
...
if __name__ == "__main__":
    ...
    F_F = [("ASML", FUTURES_id[-1], FUTURES_id[-2])] # can be a list of tuples if more futures from other stocks

    start_time = time.time()
    max_duration = 10 * 60  # 30 mins
    ...
    while True:
        ...
        while True:
            ...
            for stock_id, future_id, future_id_2 in F_F:
                ...
                if side != "none":
                    ...
                    vol_cover = hedge(stock_id, future_id, future_id_2, discount, discount_2, side)
